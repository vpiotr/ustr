...
UTEST_FUNC_DEF2(FormatContext, BasicUsage) {
    ...
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(true), "true");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(false), "false");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(42), "42");
}
...
UTEST_FUNC_DEF2(FormatContext, CustomBoolFormatter) {
    ...
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(true), "YES");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(false), "NO");
...
UTEST_FUNC_DEF2(FormatContext, CustomFloatFormatter) {
    ...
    UTEST_ASSERT_STR_EQUALS(result, "3.14");
    UTEST_ASSERT_TRUE(ctx.has_formatter<float>());
    
    // Test with negative value
    result = ctx.to_string(-1.234f);
    UTEST_ASSERT_STR_EQUALS(result, "-1.23");
    
    // Test with zero value
    result = ctx.to_string(0.0f);
    UTEST_ASSERT_STR_EQUALS(result, "0.00");
}
...
UTEST_FUNC_DEF2(FormatContext, MultipleFormatters) {
    ...
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(true), "TRUE");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(false), "FALSE");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(42), "INT:42");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(0), "INT:0");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(3.14f), "FLOAT:3.1");
    
    // Test default formatting for types without custom formatters
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(3.14), "3.140000"); // double uses default formatting
...
UTEST_FUNC_DEF2(FormatContext, RemoveFormatter) {
    ...
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(true), "true"); // Back to default
    UTEST_ASSERT_FALSE(ctx.has_formatter<bool>());
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(42), "INT:42"); // Int formatter should still work
    UTEST_ASSERT_TRUE(ctx.has_formatter<int>());
    
    ctx.remove_formatter<int>();
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(42), "42"); // Back to default
    UTEST_ASSERT_FALSE(ctx.has_formatter<int>());
}
...
UTEST_FUNC_DEF2(FormatContext, ClearFormatters) {
    ...
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(true), "YES");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(false), "NO");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(42), "NUM:42");
    UTEST_ASSERT_STR_EQUALS(ctx.to_string(0), "NUM:0");
